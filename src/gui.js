#!/usr/bin/env node
// QVAC Fake Baby Shower Game Idea Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29425;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/showergame") {
      try {
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Baby Shower Game Idea Generator GUI ready at http://localhost:${PORT}`);
  });
